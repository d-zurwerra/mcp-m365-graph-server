...
| Was | Wo | Wer |
|---|---|---|
| Global Admin im DEV-Tenant (r6ly) | entra.microsoft.com | Dominic |
| Azure Subscription (DZurwerra = in2success) | portal.azure.com | Michael |
| GitHub Repo (privat) angelegt | github.com/d-zurwerra/mcp-m365-graph-server | Dominic |
| GitHub PAT mit `read:packages` + `write:packages` + `repo` | github.com → Settings → Developer Settings | Dominic |
| Copilot Studio Lizenz | copilotstudio.microsoft.com | in2success |
| Resource Group `rg-oskar-mcp-env` | portal.azure.com | Michael/Dominic |
...
SCHRITT 12: Planner Tasks
→ get_planner_tasks (PLAN_ID)
→ Nur fehlende Tasks anlegen:
  → create_planner_task: "Kick-off Termin vereinbaren"
  → create_planner_task: "Zugänge einrichten"
  → create_planner_task: "Erstdokumentation erstellen"
  → create_planner_task: "Abnahme & Übergabe"
→ Berichte: "✅ Tasks angelegt"

SCHRITT 13: Abschlusszusammenfassung
✅ Gruppe/Team: [Kundenname] (Private)
✅ Owners & Members: [Namen]
✅ SharePoint Site: [webUrl]
✅ Onboarding Checkliste: angelegt
✅ Planner Plan: angelegt mit 4 Tasks
